...
There are currently no active dependency patches. Keep `patches/.gitkeep` so Docker's `COPY patches patches` step continues to work when the directory is otherwise empty.
...
- `openclaw` binary comes from npm (`openclaw@2026.5.2`), not built from source
